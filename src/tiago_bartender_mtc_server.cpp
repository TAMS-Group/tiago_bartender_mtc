...
public:
	TiagoBartender() :
		...
		as_place_pick_(nh_, "tiago_place_pick", boost::bind(&TiagoBartender::place_pick_cb, this, _1), false),
		as_place_(nh_, "tiago_place", boost::bind(&TiagoBartender::place_cb, this, _1), false),
	...
	void place_pick_cb(const tiago_bartender_msgs::PlacePickGoalConstPtr& goal)
	{
		std::map<std::string, moveit_msgs::AttachedCollisionObject> attached_objects = psi_.getAttachedObjects();
		if(attached_objects.size() != 1){
			ROS_ERROR_STREAM("Invalid request for PlacePickAction - No bottle in gripper to place");
			tiago_bartender_msgs::PlacePickResult result;
			result.result.result = tiago_bartender_msgs::ManipulationResult::INTERNAL_ERROR;
			as_place_pick_.setAborted(result, "Invalid request - No bottle in gripper");
			return;
		}
		...
		{
			auto stage = std::make_unique<stages::Connect>("move to pre-place pose", stages::Connect::GroupPlannerVector{{"arm_torso", sampling_planner}});
...

		{
			...
			wrapper->setIKFrame(
				...
				Eigen::AngleAxisd(M_PI, Eigen::Vector3d::UnitX()), "gripper_grasping_frame");
			wrapper->properties().configureInitFrom(Stage::PARENT, {"eef"});
			wrapper->properties().configureInitFrom(Stage::INTERFACE, {"target_pose"});
			t.add(std::move(wrapper));
		}
		...
		{
			auto stage = std::make_unique<stages::ModifyPlanningScene>("allow gripper->object collision");
			stage->allowCollisions(place_bottle, t.getRobotModel()->getJointModelGroup("gripper")->getLinkModelNamesWithCollisionGeometry(), false);
			t.add(std::move(stage));
		}
      ...
      // PICK
		...
		{
			auto stage = std::make_unique<stages::ModifyPlanningScene>("allow gripper->object collision");
			stage->allowCollisions(pick_bottle, t.getRobotModel()->getJointModelGroup("gripper")->getLinkModelNamesWithCollisionGeometry(), true);
			grasp->insert(std::move(stage));
		}
		...
		if(t.numSolutions() == 0){
			...
			if(ik_state->solutions().size() == 0){
				result.result.result = tiago_bartender_msgs::ManipulationResult::UNREACHABLE;
				as_place_pick_.setAborted(result, "IK failed, is the object reachable?");
			}
			else {
				result.result.result = tiago_bartender_msgs::ManipulationResult::NO_PLAN_FOUND;
	...
	void place_cb(const tiago_bartender_msgs::PlaceGoalConstPtr& goal)
	{
		std::map<std::string, moveit_msgs::AttachedCollisionObject> attached_objects = psi_.getAttachedObjects();
		if(attached_objects.size() != 1){
			ROS_ERROR_STREAM("Invalid request for PlaceAction - No bottle in gripper to place");
...

		{
			...
			p.pose.orientation.w= 1;
			p.pose.position.x= 0.9 + grasp_frame_transform_.inverse().translation().x();
			p.pose.position.y= 0.0;
			p.pose.position.z= table_height_
				+ 0.5 * bottle_height_
...
private:
	...
	actionlib::SimpleActionServer<tiago_bartender_msgs::PlacePickAction> as_place_pick_;
